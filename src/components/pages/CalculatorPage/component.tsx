...
import { CheckoutItem, NumericInput } from "../../atoms";
import { SelectBoxes } from "../../organisms";
...
export const CalculatorPage: FC = () => {
  ...
  const [numberOfPeople, setNumberOfPeople] = useState(0.0);
  const [tipPercent, setTipPercent] = useState(0.0);
  const [bill, setBill] = useState(0.0);

  useEffect(() => {
    if (!!numberOfPeople && !!bill && numberOfPeople > 0 && bill > 0) {
      let tipAmount = 0;

      if (!!tipPercent && tipPercent > 0) {
        tipAmount = (bill * (tipPercent || 1)) / numberOfPeople;
        setTipAmount(Math.round(tipAmount * 100) / 100);
      } else {
        setTipAmount(0);
      }

      const checkoutTotal = bill / numberOfPeople + tipAmount;
      setTotal(Math.round(checkoutTotal * 100) / 100);
    } else {
      setTotal(0);
      setTipAmount(0);
    }
  ...
  const handleSelectBox = useCallback((percent: number) => {
    console.log("percent", percent);
    setTipPercent(percent / 100);
  }, []);

  const handleBillChange = useCallback((bill: number) => {
    setBill(bill);
  }, []);

  const handlePeopleNumberChange = useCallback((numOfPeople: number) => {
    ...
    <>
      ...
      <div className="p-10 bg-white rounded-2xl">
        <div className="grid grid-cols-2 gap-10">
          ...
          <div className="flex flex-col justify-between px-10 py-14 rounded-2xl bg-splitter-blue-800">
            <div>
              <CheckoutItem
                heading="Tip Amount"
                subHeading="/ person"
            ...
            <button
              className="px-6 py-3 text-lg font-bold text-white uppercase border-0 rounded w-100 bg-splitter-blue-500 focus:outline-none hover:bg-splitter-blue-600"
              onClick={handleReset}
            >
              Reset
            </button>
